...
AGlobal::AGlobal()
{
	...
	static ConstructorHelpers::FObjectFinder<UBlueprint>
		Asteroid(TEXT("Blueprint'/Game/AsteroidBP.AsteroidBP'"));
	if (Asteroid.Object) {
...
void AGlobal::SpawnAsteroids() {
	...
	if (World) {
		for (int i = 0; i < 3; i++) {
			...
			float xPos = FMath::RandRange(actorLoc[0] - 800, actorLoc[0] + 800);
			float yPos = FMath::RandRange(actorLoc[1] - 800, actorLoc[1] + 800);
